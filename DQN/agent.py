...
class Agent:
    ...
    def __init__(self, state_size, action_size, bs, lr, tau, gamma, device, load=False):
        ...
        if load:
            self.Q_local.load_state_dict(torch.load('./skiing results/Skiing-ramDeterministic-v4_weights.pth'))
            self.Q_target.load_state_dict(torch.load('./skiing results/Skiing-ramDeterministic-v4_weights.pth'))
            print("model loaded")
#         else:
#             self.Q_local = Q_Network(self.state_size, self.action_size).to(device)
#             self.Q_target = Q_Network(self.state_size, self.action_size).to(device)
