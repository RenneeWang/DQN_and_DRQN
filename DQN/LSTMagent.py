...
from LSTMnetworks import *

class Agent:

    def __init__(self, state_size, action_size, bs, lr, tau, gamma, device, load=False):
        '''
        ...
        self.Q_local = LSTM_Q_Network(self.state_size, self.action_size).to(device)
        self.Q_target = LSTM_Q_Network(self.state_size, self.action_size).to(device)
        if load:
            self.Q_local.load_state_dict(torch.load('./skiing results/Skiing-ramDeterministic-v4_LSTM_weights.pth'))
            self.Q_target.load_state_dict(torch.load('./skiing results/Skiing-ramDeterministic-v4_LSTM_weights.pth'))
            print("model loaded")
#         else:
